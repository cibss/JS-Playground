...
function getQuestions() {
  ...
  request.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      questions = JSON.parse(this.responseText).questions; //.questions at the end is the name of json data
  ...
  request.open("GET", "/assets/json/quiz.json", false);
  request.send();
}
...
function displayQuestion(question) {
  ...
  question.answers.forEach(function (answer) {
    var label = document.createElement("label"); // Creating radio button element
